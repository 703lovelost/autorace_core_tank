...
def get_default_model_path() -> str:
    ...
    if not os.path.exists(model_path):
        raise FileNotFoundError(f"[SignClassifier] Model not found at {model_path}")
    return model_path
...
class SignClassifier:
    def __init__(self, model_path=None, device="cpu", input_size=32):
        ...
        if not os.path.exists(self.model_path):
            raise FileNotFoundError(f"[SignClassifier] Model file not found at {self.model_path}")
        state = torch.load(self.model_path, map_location=self.device)
        self.model.load_state_dict(state, strict=True)
        self.model.to(self.device)
        self.model.eval()

    def _to_tensor(self, bgr: np.ndarray) -> torch.Tensor | None:
