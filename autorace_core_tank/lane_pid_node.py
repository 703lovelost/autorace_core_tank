...
def detect_turn_direction(crop, brightness_threshold=120):
    """
    режет знак на 4 части
    меньше синего в верхней левой части -> поворот налево
    меньше синего в верхней правой части -> поворот направо
    """
...
class SimpleController(Node):
    ...
    def process_image(self, bgr: np.ndarray, depth: np.ndarray) -> Tuple[float, float]:
        ...
        def lane_detector(demonstration):
            ...
            Определяет координату по X левой и правой полосы по белым пикселям в 10 строке картинки снизу 
            """
        ...
        def look_for_sign(demonstration):
            """
            Ищет синий круглый знак на изображении по количеству синих пикселей
            и форме на маскированном по синему цвету изображении
            """
        ...
        # State: 1 - движение до перекрестка. На пути много резких поворотов, так что избегаем большого ускорения.
            ...
            # регулирует скорость поворота по формуле (центр_изображения - центр_полосы) / размер_полосы
            # если линия далеко от центра, считает, что она на расстоянии 300
            drive_speed = 0.1
            rotation_speed = 4
        ...
        # State: 2 - паркуемся у перекрестка, корректируемся на знак.
            ...
            # центрируемся на знак, определяя скорость поворота по формуле (центр_изображения - центр_знака) / размер_изображения
            rotation_speed = 1
            drive_speed = 0.05
        ...
        # State: 4 - поворот налево
        elif self.state == 4:
            # регулирует скорость поворота по формуле (центр_изображения - центр_полосы) / размер_полосы
            # если линия далеко от центра, считает, что она на расстоянии 300
            # если нет полос, поворачиваем налево
            drive_speed = 0.1
            rotation_speed = 4
        ...
        # State: 5 - поворот направо
        elif self.state == 5:
            # регулирует скорость поворота по формуле (центр_изображения - центр_полосы) / размер_полосы
            # если линия далеко от центра, считает, что она на расстоянии 300
            # если нет полос, поворачиваем направо
            drive_speed = 0.1
            rotation_speed = 4
